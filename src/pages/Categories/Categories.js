...
import { selectTotalProductsCount, selectLowStockCategoriesCount } from '../../store/slices/productSlice';
...
const Categories = () => {
  ...
  // محاسبه کل تعداد محصولات از مجموع کارت‌های دسته‌بندی
  const calculateTotalFromCategories = () => {
    return categories.reduce((total, category) => {
      const categoryProductCount = getProductCount(category.id);
      return total + categoryProductCount;
    }, 0);
  };

  // محاسبه تعداد دسته‌بندی‌های کم موجودی (کمتر از 2 محصول)
  const calculateLowStockCategories = () => {
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1 }}>
            <Chip 
              label={`کل محصولات: ${totalProductsCount.toLocaleString('fa-IR')}`}
              color="primary"
              variant="filled"
              sx={{ 
            ...
            />
            <Chip 
              label={`محاسبه از دسته‌بندی‌ها: ${calculateTotalFromCategories().toLocaleString('fa-IR')}`}
              color="info"
              variant="outlined"
              size="small"
              sx={{ 
            ...
            />
            <Chip 
              label={`دسته‌های کم موجودی: ${calculateLowStockCategories().toLocaleString('fa-IR')}`}
              color="warning"
              variant="filled"
              size="small"
              sx={{ 
      ...
      <Grid container spacing={3}>
        {filteredCategories.map((category) => {
          ...
          const isLowStock = productCount < 2;
          ...
          return (
            <Grid item xs={12} sm={6} md={4} lg={3} key={category.id}>
              <Card sx={{ 
                height: '100%', 
                position: 'relative',
                border: isLowStock ? '2px solid #f39c12' : 'none',
                boxShadow: isLowStock ? '0 4px 8px rgba(243, 156, 18, 0.3)' : 'none'
              }}>
                <CardContent>
                  {/* Category Icon */}
                  ...
                  <Typography variant="h6" sx={{ 
                    color: isLowStock ? '#f39c12' : '#d4af37', 
                    fontWeight: 'bold', 
                    textAlign: 'center', 
                    mb: 1,
                    fontFamily: 'Vazirmatn'
