...
// Pay customer debt - handles payment and updates transaction status
exports.payCustomerDebt = async (req, res) => {
  try {
    ...
    console.log('Parsed fields:', { amount, payment_method, notes, payment_date, is_full_payment, update_payment_method });
    ...
    let paymentStatus = 'partial';
    let updatedPaymentMethod = transaction.payment_method;
    
    if (remainingAmount <= 0) {
      paymentStatus = 'completed';
      // If this was an unpaid transaction and it's now fully paid, update payment method
      if (update_payment_method && transaction.payment_method === 'unpaid') {
    ...
    // Update payment method only if it's a full payment and was previously unpaid
    if (update_payment_method && paymentStatus === 'completed' && transaction.payment_method === 'unpaid') {
    ...
    res.status(201).json({
      ...
      data: {
        ...
        transaction: {
          ...
          remaining_amount: Math.max(0, remainingAmount),
          payment_status: paymentStatus,
          payment_method: updateData.payment_method || transaction.payment_method
        }
      },
      message: paymentStatus === 'completed' ? 'تراکنش کاملا پرداخت شد' : 'پرداخت با موفقیت ثبت شد'
    });
  } catch (error) {
...
exports.createInstallmentPlan = async (req, res) => {
  try {
    console.log('=== Installment Plan Debug ===');
    console.log('Transaction ID:', req.params.id);
    console.log('Request Body:', req.body);
    ...
    console.log('Parsed installment fields:', { number_of_installments, start_date, amount, notes });
    
    // Calculate installment amount per month
    const installmentAmount = parseFloat(amount) / parseInt(number_of_installments);
    
    // Create installments
    const installments = [];
    const startDateObj = new Date(start_date);
    
    for (let i = 0; i < parseInt(number_of_installments); i++) {
    ...
    // Update transaction status to installment
    await transaction.update({
      payment_method: 'installment',
      payment_status: 'partial'
    });
    
    console.log('Transaction updated to installment method');
    ...
    res.status(201).json({
      ...
      message: 'برنامه اقساط با موفقیت ایجاد شد'
    });
    ...
    res.status(400).json({
      success: false,
      message: 'خطا در ایجاد برنامه اقساط',
      error: error.message
    });
  }
};
